...
function DashboardsFlowDiagramInteractive({
  ...
  const activeAnnotation = DASHBOARDS_FLOW_ANNOTATIONS.find((item) => item.id === activeId);
  ...
  useEffect(() => {
    ...
    fetch(dashboardsProjectImages.flow.src, { cache: "no-store" })
      .then((response) => {
        if (!response.ok) throw new Error("dashboards flow svg fetch failed");
...
function DashboardsFlowDiagramTablet() {
...
export default DashboardsFlowDiagramTablet;
